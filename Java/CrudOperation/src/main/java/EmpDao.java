...
public class EmpDao {
	...
	public static int save(Emp e){
		...
		try{
			...
			PreparedStatement ps=con.prepareStatement("insert into user192(id,name,password,email,country,salary,designation) values (crud.nextval,?,?,?,?,?,?)");
	...
	public static int update(Emp e){
		...
		try{
			Connection con=EmpDao.getConnection();
			PreparedStatement ps=con.prepareStatement("update user192 set name=?,password=?,email=?,country=?,salary=?,designation=? where id=?");
			ps.setString(1,e.getName());
			ps.setString(2,e.getPassword());
			ps.setString(3,e.getEmail());
			ps.setString(4,e.getCountry());
