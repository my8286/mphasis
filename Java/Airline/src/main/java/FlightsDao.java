...
public class FlightsDao {
	...
	public static List<Flights> getFlights(String from,String to){
		...
		try{
			...
			PreparedStatement ps=con.prepareStatement("select f1.*,f2.fare from flights f1 inner join fare f2 on f1.flight_id=f2.flight_id where f1.origin=? and f1.destination=?");
			ps.setString(1,from);
			ps.setString(2,to);
